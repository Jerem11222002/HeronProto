...
async function seedPostsForOldUsersOptimized() {
  console.log('🌱 GENERATING POSTS FOR EXISTING USERS BASED ON INTERESTS (OPTIMIZED)\n');
  ...
  try {
    ...
    for (const user of oldUsers) {
      ...
      if (totalCreated >= remainingCapacity) {
        console.log(`\n⚠️  Reached capacity! Stopping seeding.`);
        break;
      }
      ...
      try {
        ...
        // Generate 3-5 posts per user, but limit if close to capacity
        ...
        console.log(`📌 ${user.name} (@${user.username}) - ${numPosts} posts`);
        ...
        for (let i = 0; i < numPosts; i++) {
          ...
          if (postsToInsert.length >= batchSize) {
            ...
            console.log(`  ✓ Inserted batch of ${postsToInsert.length} posts (Total: ${totalCreated})`);
            postsToInsert = [];
          }
        }
    ...
    if (postsToInsert.length > 0) {
      await Post.insertMany(postsToInsert);
      totalCreated += postsToInsert.length;
      console.log(`  ✓ Inserted final batch of ${postsToInsert.length} posts (Total: ${totalCreated})`);
    }
    ...
    console.log(`✅ User posts generation complete!`);
...
connectDB().then(() => seedPostsForOldUsersOptimized());
