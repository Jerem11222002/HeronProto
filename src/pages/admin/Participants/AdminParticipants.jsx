...
class ParticipantsErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error('ðŸ”¥ Participants component error:', error, errorInfo);
  }
...
const AdminParticipants = () => {
  ...
  const handleError = (error, type = 'fetch') => {
    console.error(`âŒ ${type.toUpperCase()} Error:`, {
        ...
        'An error occurred. Please try again.'
            ...
            const fetchData = async () => {
              try {
                ...
                console.log('ðŸŒ Fetching participants:', { url: apiUrl });
  ...
  const getFilteredParticipants = () => {
    ...
    try {
      ...
      console.error('âŒ Status update error:', {
        message: error.message,
        status: error.response?.status,
        data: error.response?.data
      });
  ...
  const columns = [
    ...
    // Uploaded files column
    {
      ...
      renderCell: (params) => {
        ...
        if (!files || files.length === 0) return <span style={{ color: '#666' }}>â€”</span>;
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
            <Box sx={{ flexGrow: 1 }}>
              ...
              <Typography variant="caption" color="textSecondary">
                {eventData.organization} â€¢ {new Date(eventData.eventDate).toLocaleDateString()}
              </Typography>
            </Box>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Chip 
