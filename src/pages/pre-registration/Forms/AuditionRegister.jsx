...
export default function AuditionRegister({ event, user }) {
  ...
  useEffect(() => {
    let mounted = true;
    // If studentId (or email/name) missing, try fetching full profile
    ...
    const fetchProfile = async () => {
      try {
        ...
        // try fallback by id if available
        try {
          if (user?.id || user?._id) {
          ...
          console.debug('[AuditionRegister] could not fetch full user profile', e?.message || e);
        }
      }
    };
    fetchProfile();
  ...
  // Debug: show schema in console to help backend/frontend verification
  console.debug('[AuditionRegister] using schema length:', schema.length, schema);
  ...
  if (!schema.length) {
    return (
      <div className="pre-register-form audition-form">
  ...
  return (
    <DynamicRegistrationForm
      ...
      className="pre-register-form audition-form"
    />
  );
}